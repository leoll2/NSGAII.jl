...
function nsga(popSize::Integer, nbGen::Integer, z::Function, init::Function ;
    ...
    return _nsga(X, Min(), lexico, popSize, nbGen, init, z, fdecode, fdecode!, fCV , pmut, fmut, fcross, seed, fplot, plotevery, showprogress ? 0.5 : Inf)
end
...
function nsga(popSize::Integer, nbGen::Integer, z::Function, bc::BinaryCoding ;
    ...
    return _nsga(X, Min(), lexico, popSize, nbGen, init, z, x->decode(x, bc), (g,f)->decode!(g, bc, f), fCV , pmut, fmut, fcross, encode.(seed, bc), fplot, plotevery, showprogress ? 0.5 : Inf)
end
...
function nsga_max(popSize::Integer, nbGen::Integer, z::Function, init::Function ;
    ...
    lexico = false)
	X = create_indiv(init(), fdecode, z, fCV)
    return _nsga(X, Max(), lexico, popSize, nbGen, init, z, fdecode, fdecode!, fCV , pmut, fmut, fcross, seed, fplot, plotevery, showprogress ? 0.5 : Inf)
end
...
function nsga_max(popSize::Integer, nbGen::Integer, z::Function, bc::BinaryCoding ;
    ...
    init = ()->bitrand(bc.nbbitstotal)
    X = create_indiv(init(), x->decode(x, bc), z, fCV)
    return _nsga(X, Max(), lexico, popSize, nbGen, init, z, x->decode(x, bc), (g,f)->decode!(g, bc, f), fCV , pmut, fmut, fcross, encode.(seed, bc), fplot, plotevery, showprogress ? 0.5 : Inf)
end
